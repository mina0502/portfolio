   ...
   if(Trans == true)
{
  ...
  int Select=map(Value,0,1023,low,high+100);
  if(Select>high)
    {
      Select=high;
    }
...
// =============================================================================
  ...
  if(cnt==1)
  {
    ...
    lcd.print("Range");
    lcd.setCursor(0,1);
    lcd.print("700 1000 1500");
    lcd.setCursor(9,0);
    int adcValue = analogRead(potentioMeterPin);
    int arrange = map(adcValue,0,1023,0,3100);
    low=(int)(arrange/100)*100;
    delay(100);
    lcd.print("L: ");
    lcd.print(low);
    lcd.print("   ");
  ...
  if(cnt==2)
  {
    ...
    if(sec==true)
    {
      lcd.setCursor(9,0);
      int adcValue = analogRead(potentioMeterPin);
      int timer = map(adcValue,0,1023,0,31);
      Timmer= timer;
      lcd.print(Timmer);
      lcd.print(" sec");
    }
