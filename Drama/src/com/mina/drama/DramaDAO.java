...
/***************** getReviewList() *************************/
		...
		public ReviewDramaBean getReviewList(String id,String title,int num){
			conn=getConnection();
			sql="select * from rvdrama where id=? and title=? and num=?";
			ReviewDramaBean rb= null;
			try{
				pstmt=conn.prepareStatement(sql);
				pstmt.setString(1,id);
...
/************************************************ comment ****************************************/
		...
		public ArrayList getRpList(int PageSize, int record_start_no, String title, int num) {

			conn = getConnection();

			sql = "select num,title,content,id,view,date from rvdrama where repl>0 and title=? and pagenum=? order by num desc limit ? offset ?";
			...
			try {
				...
				pstmt.setInt(4, record_start_no - 1);
				// 4 sql 실행
				rs = pstmt.executeQuery();
		...
		public int getRpNum(String title, int num){
			conn=getConnection();
			sql = "select count(*) from rvdrama where repl>0 and title=? and pagenum=? ";
			int total_record = 1;
			try {
				pstmt = conn.prepareStatement(sql);
				pstmt.setString(1, title);
